...
```{r}
lucas <- function(n) {
  if (n == 0) { #if its the last number return a 2 since the sequence starts w/ 2 
    return(2)}
  else if(n == 1){ #if its the 2nd to last number return 1 which is the same as fibonacci 
    return(1)}
  else {
    return(lucas(n-1) + lucas(n-2)) #sum of the two previous
     }
}
